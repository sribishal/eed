...
export default function Sha1HashGenerator() {
  const [mode, setMode] = useState<Mode>('sha1');
  ...
  const strToKeyBytes = (str: string, fmt: KeyFormat) => {
    if (fmt === 'utf8') return enc.encode(str);
    const b = hexToBytes(str);
    if (!b) return null;
    return b;
  };
  ...
  async function digestSHA1(bytes: ArrayBuffer): Promise<ArrayBuffer> {
    return await subtle().digest('SHA-1', bytes);
  }

  async function hmacSHA1(messageBytes: ArrayBuffer, keyBytes: ArrayBuffer): Promise<ArrayBuffer> {
  ...
  async function processText() {
    ...
    try {
      if (mode === 'sha1') {
        const h = await digestSHA1(source);
        present(h);
      } else {
        if (!hmacKey) {
          setHashHex('');
        ...
        const h = await hmacSHA1(source, kb.buffer.slice(0));
        present(h);
      }
    } catch (e) {
      console.error(e);
  ...
  async function processFile(f: File) {
    ...
    try {
      const buf = await f.arrayBuffer(); // read entire file (simple + reliable)
      if (mode === 'sha1') {
        const h = await digestSHA1(buf);
        present(h);
      } else {
        if (!hmacKey) {
        ...
        const h = await hmacSHA1(buf, kb.buffer.slice(0));
        present(h);
      }
  ...
  useEffect(() => {
    setVerifyOk(null);
    if (file) return; // avoid double hashing while a file is selected; user can re-hash via button
    if (!text) {
      setHashHex('');
      setHashBase64('');
      return;
  ...
  useEffect(() => {
    ...
    else if (norm.base64) expBytes = base64ToBytes(norm.base64) as Uint8Array | null;
    if (!expBytes) {
      setVerifyOk(false);
      return;
    }
  ...
  const onDownload = () => {
    ...
    const content = `Mode: ${mode === 'sha1' ? 'SHA-1' : 'HMAC-SHA1'}
Output (HEX): ${hex || '-'}
Output (Base64): ${b64 || '-'}
`;
  ...
  const pageTitle = mode === 'sha1' ? 'SHA-1 Hash Generator Online' : 'HMAC-SHA1 Generator';
    ...
    <div className="container mx-auto max-w-6xl">
      ...
      <div className="text-center mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-3">
          {mode === 'sha1' ? 'SHA-1 Hash Generator Online' : 'HMAC-SHA1 Generator Online'}
        </h1>
        <p className="text-sm sm:text-base text-muted-foreground max-w-3xl mx-auto">
          Fast, client-side {mode === 'sha1' ? 'SHA-1 hashing' : 'HMAC-SHA1 signing'} for text and files.
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div className="bg-muted/50 rounded-lg p-3 sm:p-4 flex items-start">
              <p className="text-xs sm:text-sm text-muted-foreground">
                ...
                HMAC-SHA1 is for message authentication, not password hashing. For new systems, use HMAC-SHA256/512.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="mt-6 sm:mt-8 bg-card rounded-xl shadow-lg border p-4 sm:p-6">
        <h2 className="text-lg sm:text-xl font-bold mb-4">About SHA-1</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          <div>
            <h3 className="text-base font-semibold mb-2">What is SHA-1?</h3>
            ...
            <ul className="text-muted-foreground space-y-1 text-xs sm:text-sm list-disc list-inside">
              <li>160-bit digest (20 bytes)</li>
              <li>HEX (40 chars) or Base64</li>
              <li>Deterministic & one-way</li>
            </ul>
          </div>
          <div>
            <h3 className="text-base font-semibold mb-2">Security Note</h3>
            <p className="text-muted-foreground mb-3 text-xs sm:text-sm">
            ...
            <ul className="text-muted-foreground space-y-1 text-xs sm:text-sm list-disc list-inside">
              <li>Legacy checksums and file verification</li>
              <li>Old APIs/protocols requiring SHA-1</li>
              <li>Educational/demo purposes</li>
            </ul>
          </div>
        </div>
